...
export class Order {
    async openOrder(user_id: number): Promise<number> {
        try {
            ...
            throw Error(
                `Couldn't open an order for user ${user_id}. Error ${error}`
            );
        }
    }
    async changeOrderStatus(id: number, status: orderStatus): Promise<void> {
        try {
            ...
            throw Error(
                `Couldn't change status for order ${id}. Error ${error}`
            );
        }
    }
    ...
    async addProduct(
        ...
        try {
            ...
            throw Error(
                `Couldn't add product of id ${product_id} to order ${order_id}. Error ${error}`
            );
        }
    }
    ...
    async getOrders(user_id: number): Promise<IOrder[]> {
        try {
            ...
            throw Error(
                `Couldn't get orders of user ${user_id}. Error ${error}`
            );
        }
    }
    ...
    async getCompletedOrders(user_id: number): Promise<IOrder[]> {
        try {
            ...
            console.log(error);
            throw Error(
                `Couldn't get completed orders of user ${user_id}. Error ${error}`
            );
        }
    }
}
